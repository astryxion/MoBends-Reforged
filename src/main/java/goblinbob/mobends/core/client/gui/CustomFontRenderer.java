...
public class CustomFontRenderer
{
    ...
    protected void drawSymbol(CustomFont.Symbol symbol, BufferBuilder vertexBuffer, int x, int y)
    {
        ...
        vertexBuffer.vertex((double) x, (double) (y), 0)
                .uv(textureX, textureY + textureHeight).endVertex();
        vertexBuffer.vertex((double) (x + width), (double) (y), 0)
                .uv(textureX + textureWidth, textureY + textureHeight).endVertex();
        vertexBuffer.vertex((double) (x + width), (double) (y - height), 0)
                .uv(textureX + textureWidth, textureY).endVertex();
        vertexBuffer.vertex((double) x, (double) (y - height), 0)
    ...
    public void drawText(String textToDraw, int x, int y)
    {
        ...
        goblinbob.mobends.api.platform.PlatformServices.get().setPositionTexShader();
        Minecraft.getInstance().getTextureManager().bind(this.font.resourceLocation);
