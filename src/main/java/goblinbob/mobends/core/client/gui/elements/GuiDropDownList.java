...
public class GuiDropDownList<T> implements IObservable
{
	...
	public boolean handleMouseScroll(double delta)
	{
		...
		int mouseWheelRoll = delta > 0 ? -1 : (delta < 0 ? 1 : 0);
	...
	public void display()
	{
		...
		String text = noValue ? "None"
				: this.fontRenderer.plainSubstrByWidth(getSelectedEntry().getLabel(), this.getWidth() - 20);
		this.fontRenderer.drawShadow(new com.mojang.blaze3d.matrix.MatrixStack(), text, x + 5, y + 4, noValue ? 0x999999 : 0xe2e2e2);
		...
		// Arrow icon
		Minecraft.getInstance().getTextureManager().bind(GuiBendsMenu.ICONS_TEXTURE);
		...
		if (dropped)
		{
			...
			for (int i = 0; i < elements; i++)
			{
				...
				String name = noValue ? "None"
						: this.fontRenderer.plainSubstrByWidth(entries.get(entryID).getLabel(), getWidth());
				this.fontRenderer.drawShadow(new com.mojang.blaze3d.matrix.MatrixStack(), name, x + 3, y + HEIGHT + i * ELEMENT_HEIGHT + 2,
