...
public class GuiPopUp
{
    ...
    public GuiPopUp(String title, int width, int height, ButtonProps[] buttonProps)
    {
        ...
        this.width = width;
        this.height = height;
        this.title = goblinbob.mobends.core.util.GuiUtil.wrapText(fontRenderer, title, width - 20);
    ...
    public void display(int mouseX, int mouseY, float partialTicks)
    {
        ...
        {
            fontRenderer.drawShadow(new MatrixStack(), line, x + (width - fontRenderer.width(line)) / 2, y + yOffset, 0xffffff);
            yOffset += 9;
        }
    ...
    public void mouseClicked(int mouseX, int mouseY, int state)
    {
        for (Button button : buttons)
        {
            if (button.buttonUI.mousePressed(Minecraft.getInstance(), mouseX, mouseY))
            {
    ...
    public static class Button
    {
        ...
        {
            this.props = props;
            int labelWidth = fontRenderer.width(props.label);
